package bg.minij.tools.ast_lower;
...
public class AstLowerMain {
    ...
    // ─── library (external corelib) mode ──────────────────────────────────
    // Обектите пазят compile-time class index в header-а (st_hdr) и virtual
    // dispatch индексира глобалния масив `vtables:`. За да съвпаднат номерата
    // между app-o и предварително компилираната corelib библиотека, corelib
    // build-ът пише classmap (idx → class); app-o го зарежда с --classmap и
    // "заковава" тези индекси, а класовете му получават следващите свободни.
    Set<String> externalDirs = new HashSet<>();                // absolute roots: resolve, don't emit
    Set<String> extraExternals = new HashSet<>();              // from --externals= (driver-намек)
    Set<String> externalClasses = new HashSet<>();             // classes declared in external units
    Map<String, Integer> classMap = new LinkedHashMap<>();     // class → фиксиран index (--classmap)
    String emitClassmapPath = null;
...
public static void run(String[] args) throws Exception {
        ...
        Set<String> extDirs = new HashSet<>();
        Set<String> extClasses = new HashSet<>();
        Map<String, Integer> classMap = new LinkedHashMap<>();
        String emitClassmap = null;
        for (int i = 0; i < args.length; i++) {
            ...
            else if (a.startsWith("--external-dir=")) extDirs.add(a.substring(15));
            else if (a.startsWith("--externals=")) extClasses.addAll(Arrays.asList(a.substring(12).split(",")));
            else if (a.startsWith("--classmap=")) {
                String cm = a.substring(11);
                int sep = cm.lastIndexOf(':');
                String simple = sep < 0 ? cm : cm.substring(0, sep);
                int idx = sep < 0 ? -1 : Integer.parseInt(cm.substring(sep + 1));
                if (!simple.isEmpty() && idx >= 0) classMap.put(simple, idx);
            }
            else if (a.startsWith("--emit-classmap=")) emitClassmap = a.substring(16);
            else if (in == null) in = a;
            else out = a;
        }
        if (in == null || out == null) { System.err.println("usage: ast-lower [-I dir]... [--external-dir=dir] [--classmap=idx:cls,...] [--emit-classmap=file] <in.mj> <out.lir>"); System.exit(1); }
        ...
        for (String d : extDirs) m.externalDirs.add(Path.of(d).toAbsolutePath().normalize().toString());
        m.extraExternals.addAll(extClasses);
        m.classMap.putAll(classMap);
        m.emitClassmapPath = emitClassmap;
        m.addUnit(mainCu, in.equals("-") ? null : Path.of(in).toAbsolutePath().normalize().toString());
        m.loadClosure();
        m.externalClasses.addAll(m.extraExternals);
        ...
        m.buildVtables();
        if (m.emitClassmapPath != null) {
            StringBuilder cm = new StringBuilder();
            List<Map.Entry<String, Integer>> es = new ArrayList<>(m.classIndex.entrySet());
            es.sort(Map.Entry.comparingByValue());
            for (Map.Entry<String, Integer> e : es) cm.append(e.getValue()).append(" ").append(e.getKey()).append("\n");
            Files.writeString(Path.of(m.emitClassmapPath), cm.toString());
        }
        for (Object td : types) if (td instanceof Java.ClassDeclaration cd) {
            String cn = getStr(cd, "name");
            if (cn != null && m.externalClasses.contains(cn)) continue;
            m.emitMethods(cd);
        }
    ...
    void addUnit(Java.AbstractCompilationUnit cu, String path) {
        ...
        Unit u = new Unit();
        u.cu = cu; u.path = path;
        u.external = isExternalPath(path);
        ...
        for (Object td : members(cu)) if (td instanceof Java.ClassDeclaration cd) {
            ...
            if (n == null) continue;
            if (u.external) externalClasses.add(n);
    ...
    boolean isExternalPath(String path) {
        if (path == null || externalDirs.isEmpty()) return false;
        String ab = Path.of(path).toAbsolutePath().normalize().toString();
        for (String d : externalDirs) {
            if (ab.equals(d) || ab.startsWith(d + File.separator)) return true;
        }
        return false;
    }
     ...
     *  aligned по тип (i32→4, i64/f64/ptr→8). В library mode external класовете
     *  взимат фиксиран index от classmap-а (трябва да съвпада с corelib build-а). */
    void collectClass(Java.ClassDeclaration cd) {
        ...
        Integer fixed = classMap.get(cn);
        int idx = fixed != null ? fixed : classIndex.size();
        ...
        if (members instanceof List<?> ml) for (Object mb : ml) {
            ...
            if (vds instanceof Object[] arr) for (Object vd : arr) {
                ...
                if (isStatic) {
                    boolean extCls = externalClasses.contains(cn);
                    ...
                    if (ini != null) {
                        ...
                        Long cv = (Long) kv;
                        if (!extCls) statics.add(new Ir.Static(sym, ft.equals("i32") ? 4 : 8, cv));

                        staticFields.computeIfAbsent(cn, k -> new LinkedHashMap<>()).put(name, new Object[]{ ft, sym, jt });
                        continue;
                    }
                    if (!extCls) statics.add(new Ir.Static(sym, ft.equals("i32") ? 4 : 8));
                    staticFields.computeIfAbsent(cn, k -> new LinkedHashMap<>()).put(name, new Object[]{ ft, sym, jt });
                    continue;
                }
    ...
    void buildVtables() {
        ...
        for (Map.Entry<String, Integer> e : classIndex.entrySet()) {
            ...
            vt.label = "vt_" + cls;
            vt.index = e.getValue();
            vt.external = externalClasses.contains(cls);
