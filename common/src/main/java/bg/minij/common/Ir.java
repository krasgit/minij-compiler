...
public class Ir {
    ...
    public static class VTable {
        ...
        public int index;      // position in the global `vtables` array (object-header kind/class)
        public boolean external; // label defined in a linked library, not in this unit
    }
        ...
        void go() {
            ...
            if (!p.vtables.isEmpty()) {
                ...
                for (VTable v : p.vtables) {
                    sb.append("  ").append(v.label).append("@").append(v.index);
                    if (v.external) sb.append("@X");
                    sb.append(" = ");
    ...
    public static class Reader {
        ...
        void readVtables() { next();
            while (!peek().trim().startsWith("]")) {
                ...
                String lhs = ln.substring(0, eq).trim();
                vt.label = lhs;
                if (lhs.contains("@")) {
                    String[] lp = lhs.split("@");
                    vt.label = lp[0];
                    for (int i = 1; i < lp.length; i++)
                        if (lp[i].equals("X")) vt.external = true;
                        else vt.index = Integer.parseInt(lp[i]);
                }
