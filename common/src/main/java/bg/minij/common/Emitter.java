    ...
    void rodata() {
        ...
        if (!prog.vtables.isEmpty()) {
            ...
            for (Ir.VTable vt : prog.vtables) {
                if (vt.external) continue; // defined in a linked library (corelib)
                out.append("    .globl ").append(vt.label).append("\n");
                out.append(vt.label).append(":\n");
                for (String s : vt.syms) out.append("    .quad ").append(s).append("\n");
            }
            out.append("    .p2align 3\n").append("vtables:\n");
            // плътен масив по index: дупки (класове от classmap-a, които самият
            // app не дефинира/ползва) се пълнят с нула, за да не се разместят
            // следващите индекси (обектите индексират по header class index).
            Map<Integer, Ir.VTable> byIndex = new HashMap<>();
            int maxIdx = 0;
            for (Ir.VTable vt : prog.vtables) { byIndex.put(vt.index, vt); maxIdx = Math.max(maxIdx, vt.index); }
            for (int i = 0; i <= maxIdx; i++) {
                Ir.VTable vt = byIndex.get(i);
                out.append("    .quad ").append(vt == null ? "0" : vt.label).append("\n");
            }
        }
